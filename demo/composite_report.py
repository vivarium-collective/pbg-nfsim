...
from pbg_nfsim.composites import (
    _COMPOSITES_DIR,
    build_composite,
...
def run(spec_name: str, n_steps: int = 20, interval: float | None = None):
    ...
    viz_html = (state.get("stores") or {}).get("viz_html", "") if isinstance(state, dict) else ""
    if not viz_html and isinstance(state, dict):
        # Some composites wire viz_html as a top-level store rather than
        # nested under "stores".
        viz_html = state.get("viz_html", "") or ""
...
def main(spec_name: str | None = None, n_steps: int = 20,
    ...
    if spec_name is None:
        ...
        if not specs:
            raise SystemExit("No *.composite.yaml found in pbg_nfsim/composites/")
        spec_name = specs[0]
